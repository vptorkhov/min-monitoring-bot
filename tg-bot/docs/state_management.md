...
Теперь весь код использует **единый менеджер состояний** —
`src/bot/state-manager.ts`.

Все другие модули подключаются либо напрямую к `stateManager`, либо через
простые обёртки (`user-state.ts`), которые сохраняются для обратной
совместимости.
