...
export class SessionService {
  ...
  public async startSession(
    ...
    if (!device.is_personal && device.warehouse_id !== courier.warehouse_id) {
      return {
        success: false,
        error: "Устройство не принадлежит вашему складу",
      };
    }
    ...
    if (!device.is_personal) {
      ...
      if (activeByDevice) {
        return {
          success: false,
          error: "Этот Велосипед уже занят другим курьером",
        };
      }
    }
  ...
  public async getActiveSessionsByWarehouse(
    warehouseId: number,
  ): Promise<ActiveWarehouseSessionView[]> {
  ...
  public async getSessionsHistoryByWarehouseAndStartDateRange(
    ...
    const sessions =
      await this.sessionRepo.getHistoryByWarehouseAndStartDateRange(
        warehouseId,
        startDateFromUtc,
        startDateToUtc,
      );
