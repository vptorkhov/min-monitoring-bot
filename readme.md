...
└── tg-bot/                             # Telegram-бот
    ...
    └── src/                            # Исходный код приложения
        ...
        │   │   ├── index.ts            # Регистрация всех команд (/start, /set_warehouse и др.)
        ...
        │   └── states.constant.ts      # Состояния регистрации и выбора склада
        │
...
- Прикрепление к складу
  - Курьер выбирает команду `/set_warehouse`
  - Бот показывает список активных складов с нумерацией
  - Курьер вводит номер склада, чтобы прикрепиться
  - Поле `warehouse_id` обновляется в таблице `couriers`
