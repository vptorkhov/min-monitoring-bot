...
| `/admin_courier_history` | Подкоманда в сценарии `/admin_edit_couriers`: показывает последние 50 сессий курьера и предлагает вывод полной истории по ответу `ДА` (строго верхний регистр). |
...
| `/superadmin_courier_history` | Подкоманда в сценарии `/superadmin_edit_couriers`: история сессий выбранного курьера с preview 50 записей и полной историей по `ДА`. |
