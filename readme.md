...
└── tg-bot/                             # Telegram-бот
    ...
    └── src/                              # Исходный код приложения
        ...
        │   ├── commands.constant.ts      # Список поддерживаемых команд бота (/start, /set_warehouse, /take_sim, /return_sim, /cancel)
...
- **Прикрепление курьеров к складам** — через `/set_warehouse` с выбором из активных складов; запрещено при активной сессии
...
| status_comment | TEXT        | Подробное описание статуса (повреждения)   |
...
| `/set_warehouse` | Позволяет курьеру выбрать склад для прикрепления. Бот показывает список активных складов и просит ввести номер выбранного склада (запрещено при активной сессии)   |
