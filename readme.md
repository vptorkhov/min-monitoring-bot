...
└── tg-bot/                             # Telegram-бот
    ...
    └── src/                              # Исходный код приложения
        ...
        │   │   ├── admin.ts              # Команды /admin, /exit_admin, /admin_login, /admin_register
        ...
        │   ├── admin.repository.ts       # Запросы к таблице admins (проверка логина, создание заявки админа)
        │   └── types/
        ...
        │   ├── admin.service.ts          # Логика регистрации администраторов (валидация + хеширование пароля)
...
- **Скрытый админский режим** — команда `/admin` в любой момент останавливает текущий курьерский сценарий и переводит пользователя в admin-mode; до реализации авторизации разрешены только `/admin_login`, `/admin_register` и `/exit_admin`
- **Возврат в курьерский режим** — команда `/exit_admin` выключает admin-mode и восстанавливает следующий шаг по состоянию курьера (регистрация, выбор склада, активная сессия, обычное меню)
...
| `/admin_login`     | Заглушка для будущего входа администратора (работает только внутри admin-mode).                                                                                                                                                          |
